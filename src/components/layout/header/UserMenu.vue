  ...
  <div class="relative" ref="dropdownRef">
    ...
    >
      <div>
        ...
        <span class="mt-0.5 block text-theme-xs text-brand-500 capitalize">
          {{ auth.user?.companyRole }} · {{ auth.user?.companyName }}
        </span>
      </div>

      <ul class="flex flex-col gap-1 pt-4 pb-3 border-b border-gray-200 dark:border-gray-800">
        <li v-if="auth.isOwnerOrAdmin">
          <router-link
            to="/admin/settings"
            @click="closeDropdown"
      ...
      <button
        @click="signOut"
        class="flex items-center gap-3 px-3 py-2 mt-3 font-medium text-gray-700 rounded-lg group text-theme-sm hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-white/5 w-full"
      >
