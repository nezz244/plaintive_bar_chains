...
interface StockItem {
  ...
  buying_price: number // changed to number for calculations
}

const warehouseStock = ref<StockItem[]>([])

// Fetch warehouse stock from API
const fetchWarehouseStock = async (): Promise<void> => {
  try {
...
const totalStockValue = computed(() =>
  warehouseStock.value.reduce((sum, item) => sum + item.cases_available * item.buying_price, 0)
)

// Compute radial chart percentage (example: total units / max capacity)
const warehouseStockPercentage = computed(() => {
  ...
  return maxUnits > 0 ? (totalUnits / maxUnits) * 100 : 0
})
